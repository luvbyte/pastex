<template>
  <div class="relative container mx-auto max-w-4xl p-2 overflow-y-auto">
    ...
    <div class="flex flex-col gap-4">
      ...
      <div class="flex gap-2 items-center">
        ...
        <div class="w-full flex items-center gap-2">
          <input v-model="isPrivate" type="checkbox" class="toggle" />
          <p class="text-xs opacity-60">Hide from recents</p>
        </div>
      </div>
    ...
    <!-- Recents -->
    <div class="opacity-60 divider">Recent pastes</div>

    <div
      v-if="recentPastes.length"
      class="flex flex-col divide-y divide-base-300 border border-base-300 rounded-box"
    >
      <router-link
      ...
      >
        <div class="flex items-center justify-between gap-4">
          <h1 class="font-medium truncate flex items-center gap-2">
            {{ paste.title || "Untitled" }}
            <span v-if="paste.crypto">
              <svg
        ...
        <p class="text-sm opacity-60 mt-1 line-clamp-1">
          {{ paste.crypto ? "Encrypted paste" : paste.content.slice(0, 100) }}
        </p>
      </router-link>
    </div>

    <p v-else class="text-center text-sm opacity-60 py-4">
...
<script setup>
  ...
  onMounted(async () => {
    try {
      recentPastes.value = await getRecentPastes();
    } catch (err) {
      console.error(err);
    }
  });
